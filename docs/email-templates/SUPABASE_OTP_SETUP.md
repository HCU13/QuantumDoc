...
### 4. **Redirect URL'leri Kaldır**
- **Site URL** ayarını temizle (OTP için gerekli değil)
- **Redirect URLs** listesini temizle
...
### 6. **URL Configuration (OTP için)**
Template'de `{{ .ConfirmationURL }}` yerine sadece `{{ .Token }}` kullanıyoruz.
